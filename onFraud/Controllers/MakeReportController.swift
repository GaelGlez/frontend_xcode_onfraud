...
@MainActor
final class MakeReportController: ObservableObject {
    ...
    func loadCategories() async {
        do {
            ...
            categoriesErrorMessage = nil // limpiar error si la carga fue exitosa
    ...
    func sendReport() async {
        ...
        isSubmitting = true
        if let response = await reportClient.createReport(form: form) {
            successMessage = "Reporte creado con éxito: \(response.title)"
            showSuccess = true
        } else {
            showError = true
